...
- (mbgl::LinearRing<double>)linearRingWithNumberOfVertices:(NSUInteger)numberOfVertices {
    ...
    CLLocationDistance radius = self.radius;
...
- (mbgl::Polygon<double>)polygon {
    ...
    NSUInteger maximumSides = ceil(circumference / maximumEdgeLength);
    ...
    NSUInteger numberOfVertices = ceil(MIN(maximumSides, maximumVertices));
    
    mbgl::Polygon<double> polygon;
    polygon.push_back([self linearRingWithNumberOfVertices:numberOfVertices]);
    return polygon;
}
...
- (mbgl::Annotation)annotationObjectWithDelegate:(id <MGLMultiPointDelegate>)delegate {
    ...
    annotation.color = { [delegate fillColorForCircleAnnotation:self] };
